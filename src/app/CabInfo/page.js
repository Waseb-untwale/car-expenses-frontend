...
//       try {
//         const wsUrl = "ws://car-expenses-backend.vercel.app"; // Update with your WebSocket server URL
//         console.log("Connecting to WebSocket server at:", wsUrl);
//         wsRef.current = new WebSocket(wsUrl);
...
// Dynamically import components that use browser-only libraries
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        setCompanyLogo("https://via.placeholder.com/150x50?text=Company+Logo")
        setSignature("https://via.placeholder.com/150x50?text=Signature")
        setCompanyName("Demo Company")
        setCompanyInfo("123 Business Street, Business City, 12345")
        setInvoiceNumber(generateInvoiceNumber("Demo Company"))
      } catch (err) {
        console.error("Failed to fetch admin data:", err)
      }
    }
  ...
  useEffect(() => {
    const fetchAssignedCabs = async () => {
      ...
      try {
            ...
            cab: {
              ...
              tripDetails: {
                ...
                fuel: {
                  ...
                  type: "Diesel",
                  receiptImage: [
                    "https://via.placeholder.com/100x100?text=Receipt1",
                    "https://via.placeholder.com/100x100?text=Receipt2",
                  ],
                  transactionImage: ["https://via.placeholder.com/100x100?text=Transaction1"],
                },
                tyrePuncture: {
                  repairAmount: [400],
                  image: ["https://via.placeholder.com/100x100?text=Tyre1"],
                },
                vehicleServicing: {
                  requiredService: true,
                  details: "Regular maintenance",
                  amount: [2000],
                  image: ["https://via.placeholder.com/100x100?text=Service1"],
                  receiptImage: ["https://via.placeholder.com/100x100?text=ServiceReceipt1"],
                },
                otherProblems: {
                  details: "Minor repairs",
                  amount: [300],
                  image: ["https://via.placeholder.com/100x100?text=Problem1"],
                },
              },
            },
            driver: {
            ...
            cab: {
              ...
              tripDetails: {
                ...
                fuel: {
                  ...
                  type: "Petrol",
                  receiptImage: [
                    "https://via.placeholder.com/100x100?text=Receipt3",
                    "https://via.placeholder.com/100x100?text=Receipt4",
                  ],
                  transactionImage: ["https://via.placeholder.com/100x100?text=Transaction2"],
                },
              },
            },
            driver: {
        ...
        setCabData(mockData[0].cab)
        setCabDetails(mockData)
        setFilteredCabs(mockData)

        // Mock route coordinates
        const routes = {}
        const driverRoutesMap = {}

        for (const cab of mockData) {
          if (cab.cab?.tripDetails?.location?.from && cab.cab?.tripDetails?.location?.to) {
            // Mock route data
            const routeData = {
              from: {
                lat: cab.driver.location.latitude - 0.5,
                lng: cab.driver.location.longitude - 0.5,
                name: cab.cab.tripDetails.location.from,
              },
              to: {
                lat: cab.driver.location.latitude + 0.5,
                lng: cab.driver.location.longitude + 0.5,
                name: cab.cab.tripDetails.location.to,
              },
            }
  ...
  const handleLocationClick = (item) => {
    ...
    setSelectedDriver({
      ...
      cab: {
        ...
        // Ensure route information is properly formatted
        location: {
          from: item.cab?.tripDetails?.location?.from || "Pune",
          to: item.cab?.tripDetails?.location?.to || "Pune",
          totalDistance:
            item.cab?.tripDetails?.location?.totalDistance ||
            calculateRouteDistance(item.cab?.tripDetails?.location?.from, item.cab?.tripDetails?.location?.to),
        },
      },
                      ...
                      <tr key={index} className="border-b border-gray-600 hover:bg-gray-600 transition-colors">
                        ...
                        <td className="p-3">
                          <select
                            className="border p-1 rounded bg-gray-800 text-white"
                            onChange={(e) =>
                              e.target.value && openModal(e.target.value, item.cab.tripDetails[e.target.value])
                            }
                          >
                            <option value="">Select</option>
                            <option value="fuel">Fuel</option>
                        ...
                        <td className="p-2">
                          <PDFDownloader
                            ...
                            companyName={subCompanyName}
                            invoiceNumber={
                              invoiceNumber || `${derivePrefix(subCompanyName)}-${String(index + 1).padStart(5, "0")}`
                            }
                            invoiceDate={new Date().toLocaleDateString("en-IN")}
                            cabData={cabData}
                            fileName={`Invoice-${item?.cab?.cabNumber}.pdf`}
              ...
              <div className="p-4 bg-gray-100">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-800">
                      Distance:{" "}
                      {selectedDriver.tripDetails?.location?.totalDistance ||
